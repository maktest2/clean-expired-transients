<?php
/**
 * Class Tests_Single_SiteTransient
 *
 * @package Clean_Expired_Transients
 */

/**
 * Test case for site transients on the single site.
 */
class Tests_Single_SiteTransient extends WP_UnitTestCase {
	/**
	 ...
	 */
	 public function test_without_cleaning() {
 		...
 		$this->assertEquals( get_site_option( '_site_transient_key1' ), 'value1' );

 		// Direct retrieval of transient timeout should be integer and less than current time`
 		$raw_key1_timeout_before_sleep = get_site_option( '_site_transient_timeout_key1' );
 		...
 		// Direct retrieval of transient timeout should be integer and less than current time`
 		$raw_key1_timeout_after_sleep = get_site_option( '_site_transient_timeout_key1' );
 		...
 		$this->assertFalse( get_site_option( '_site_transient_key1' ) );

 		// Direct retrieval of expired transient timeout should be false
 		$this->assertFalse( get_site_option( '_site_transient_timeout_key1' ) );
 	}
 	...
 	public function test_with_cleaning() {
 		...
 		// Direct retrieval of transient value should return setted value
 		$this->assertEquals( get_site_option( '_site_transient_key2' ), 'value2' );

 		// Direct retrieval of transient timeout should be integer and less than current time`
 		$raw_key2_timeout_before_sleep = get_site_option( '_site_transient_timeout_key2' );
 		...
 		$this->assertFalse( get_site_option( '_site_transient_key2' ) );

 		// Direct retrieval of expired and cleaned transient timeout should be false
 		$this->assertFalse( get_site_option( '_site_transient_timeout_key2' ) );
